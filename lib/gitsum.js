...
(function() {
  ...
  if (__indexOf.call(server_commands, mode) >= 0) {
        ...
        for (_i = 0, _len = temp_array.length; _i < _len; _i++) {
          ...
          if (~line.indexOf('new file mode')) {
            ...
            if (~prev.indexOf('gitsum/')) {
              ...
              exec('[ -f ./gitsum_tokens/' + file + ' ] && echo "1" || echo "0"', function(err, stdout, stderr) {
                if (stdout.trim() === '1') {
                  console.log('commit verified!');
                  return exec('rm -rf ./gitsum_tokens', function(err, stdout, stderr) {
                    console.log('cherry-picking commit ' + commit + '...');
                    return exec('git cherry-pick ' + commit, function(err, stdout, stderr) {
                      console.log('done!');
                      return res.send('hi');
                    });
                  });
    ...
    app.get('/gitsum', function(req, res) {
      ...
      return exec('rm -rf ./gitsum_tokens', function(err, stdout, stderr) {
        return exec('mkdir ./gitsum_tokens', function(err, stdout, stderr) {
          return exec('touch ./gitsum_tokens/' + file_name, function(err, stdout, stderr) {
    ...
    reques = http.request(options, function(response) {
      ...
      return response.on('end', function() {
        ...
        return exec('rm -rf ./gitsum', function(err, stdout, stderr) {
          return exec('git rm ./gitsum/*', function(err, stdout, stderr) {
            return exec('mkdir ./gitsum', function(err, stdout, stderr) {
              console.log('creating token file ./gitsum/' + str);
              return exec('touch ./gitsum/' + str, function(err, stdout, stderr) {
